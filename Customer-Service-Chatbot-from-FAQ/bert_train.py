...
# Ignore warnings and set TensorFlow log level to 3
...
def load_data(path, sheet=None):
    """
    Load data from Excel file and preprocess it.
    """
...
def convert_data_into_input_example(data):
    """
    Convert data into InputExamples.
    """
    input_examples = []
    for i in range(len(data)):
...
def transfer(bdset):
    """
    Transfer data into input IDs, attention masks, token type IDs, and labels.
    """
...
def train(train_ds):
    ...
    optimizer = tf.keras.optimizers.Adam(
        learning_rate=1e-5, epsilon=1e-08, clipnorm=1.0)
    ...
    my_callbacks = [
        ...
        ModelCheckpoint(model_savepath, monitor='accuracy', patience=5,
...
if __name__ == '__main__':
    ...
    parser.add_argument("-fp", "--file_path", required=True,
                        help="filepath of the standard question", type=str)
    parser.add_argument("-sn", "--sheet_name",
                        help="sheet name of the standard question", type=str)
    parser.add_argument("-mp", "--model_path", required=True,
                        help="path to store the model", type=str)
    ...
    bert_train_dataset = glue_convert_examples_to_features(
        examples=train_input_examples, tokenizer=tokenizer, max_length=MAX_SEQ_LENGTH, task='mrpc', label_list=label_list)
    x_train, y_train = transfer(bert_train_dataset)
    train_ds = tf.data.Dataset.from_tensor_slices(
        (x_train[0], x_train[1], x_train[2], y_train)).map(example_to_features).shuffle(100).batch(32)
    train(train_ds)
